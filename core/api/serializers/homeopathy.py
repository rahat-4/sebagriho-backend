...
class HomeopathicPatientSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        with transaction.atomic():
            ...
            user = User.objects.get_or_create(phone=phone, defaults={**user_data})[0]
...
class HomeopathicAppointmentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        with transaction.atomic():
            ...
            last_appointment = (
                HomeopathicAppointment.objects.select_for_update()
                .filter(organization=organization, appointment_date=today)
                .order_by("-serial_number")
                .first()
            )
            next_serial = (
                (last_appointment.serial_number + 1) if last_appointment else 1
            )
